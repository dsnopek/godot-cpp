...
def _get_api_file(extension_dir, api_version):
    if api_version is None or api_version == api_versions[-1]:
    ...
    if not os.path.exists(path):
        raise Exception("Cannot find `%s` file for api_version %s" % (filename, api_version))
